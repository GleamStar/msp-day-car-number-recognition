...
class ImagePreparation:
    """Image Preparation Using OpenCV Library"""

    def __init__(self):
        self.name = "CV2 Image Preparation"
            ...
            if (aspect_ratio >= aspect_ratio_range[0] and aspect_ratio <= aspect_ratio_range[1]):
                if ((height * width > area_range[0]) and (height * width < area_range[1])):
                    ...
                    if abs(point[0] - opposite_point[0]) > 0:
                        ...
                        tmp_angle = self.rad_to_deg(math.atan(tmp_angle))
    ...
    def process_image(self, name, debug, **options):
        se_shape = (16, 4)
        ...
        for contour in contours:
            ...
            if self.validate_contour(contour, gray, aspect_ratio_range, area_range):
                ...
                if edge_density > 0.5:
                    ...
                    self.write_image(input_image, box)

        return input_image

    def write_image(self, input_image, box_contour):
        x, y, w, h = cv2.boundingRect(box_contour)
        crop = input_image[y:y + h, x:x + w]
        cv2.imwrite('./filtered/sofwinres.png', crop)
